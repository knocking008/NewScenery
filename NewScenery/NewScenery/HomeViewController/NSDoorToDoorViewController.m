...
@interface NSDoorToDoorViewController ()<UITextFieldDelegate,UWDatePickerViewDelegate,UITextViewDelegate,UITextFieldDelegate>
@property (nonatomic, strong)NSMutableArray *dataArray;
@property (nonatomic, strong)UWDatePickerView *pickerView;
@end

@implementation NSDoorToDoorViewController
...
- (void)initUI{
    self.navigationItem.title = @"预约见面咨询";
    ...
    [self.timeTF addGestureRecognizer:timeTap];
    //===
    ...
    self.questionTFView.delegate = self;
    self.nameTF.delegate = self;
    self.nameTF.tag = 10;
    self.phoneTF.delegate = self;
    self.phoneTF.tag = 20;
    self.doorToDoorAddressTF.delegate = self;
    self.doorToDoorAddressTF.tag = 30;
    
}

- (IBAction)payClick:(id)sender {
    if (self.serviceTimeTF.text.length == 0) {
        [self alertView:@"请选择咨询时间"];
        return;
    }
    if (self.doorToDoorAddressTF.text.length == 0) {
        [self alertView:@"请输入咨询地址"];
        return;
    }
    if (self.questionTFView.text.length == 0) {
...
- (void)getNetDatas{
    [NSHTTPTool ServiceCanDoorToDoorGetNetDatas:^(NSArray *data) {
        [self.dataArray removeAllObjects];
        [self.dataArray addObjectsFromArray:data];
        dispatch_async(dispatch_get_main_queue(), ^{
            NSCanDoorToDoorModel *model = [self.dataArray firstObject][[self.designerType intValue]-1];
